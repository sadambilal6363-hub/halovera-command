...
export default async function Page(){
 ...
 <div className="tableCard">{rows.length?rows.map(x=><div className="row" key={x.id}><div><Link href={"/projects/"+x.id}><b>{x.name}</b></Link><small>{x.category||x.domain||"—"}</small></div><div><span>{Number(x.budget||0).toLocaleString("ar-AE")} د.إ</span><small>{x.status||"planning"} · {x.priority||"normal"}</small></div></div>):<div className="emptyState">لا توجد مشاريع بعد. أضف أول مشروع من النموذج أعلاه.</div>}</div></main>
}