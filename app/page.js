...
async function dashboard() {
  try {
    ...
    const [projects, clients, leads, tasks, finance] = await Promise.all([
      ...
      db.prepare("SELECT COUNT(*) n FROM leads WHERE status NOT IN ('lost','closed')").first(),
...
export default async function Home(){
  ...
  return <main>
    ...
    <section className="projects">{d.projects.length?d.projects.map(p=><article className="project" key={p.id}><div className="projectTop"><div><span className="type">{p.category||"مشروع"}</span><h3>{p.name}</h3></div><span className={"status "+(p.status==="completed"?"done":"")}>{p.status||"planning"}</span></div><div className="projectFoot"><span>{p.domain||p.live_url||"بدون رابط"}</span><strong>{money(p.budget)}</strong></div></article>):<article className="project empty"><h3>لا توجد مشاريع بعد</h3><p>أضف أول مشروع من قسم المشاريع.</p></article>}</section>
    ...
    <nav className="bottomNav"><Link className="active" href="/">القيادة</Link><Link href="/clients">العملاء</Link><Link href="/leads">المبيعات</Link><Link href="/projects">المشاريع</Link><Link href="/finance">المالية</Link></nav>
  </main>
}